  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Platform-wide analytics for Afrisinc Identity</p>
      </div>
            ...
            {isLoading ? (
              ...
              <ChartContainer config={{ count: { label: "Enrollments", color: "hsl(var(--primary))" } }} className="h-64">
                <BarChart data={data?.enrollmentsByProduct}>
                  ...
                  <XAxis dataKey="product" tick={{ fill: "hsl(var(--muted-foreground))" }} />
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <div className="flex gap-2">
            {(["7d", "30d", "90d"] as const).map((r) => (
              <Button key={r} variant={growthRange === r ? "default" : "outline"} size="sm" onClick={() => setGrowthRange(r)}>
          ...
          {growthLoading ? (
            ...
            >
              <LineChart data={growthData}>
                ...
                <ChartTooltip content={<ChartTooltipContent />} />
                <Legend />
                <Line type="monotone" dataKey="newUsers" stroke="hsl(var(--primary))" dot={false} strokeWidth={2} />
                <Line type="monotone" dataKey="newAccounts" stroke="hsl(var(--secondary))" dot={false} strokeWidth={2} />
                <Line type="monotone" dataKey="newEnrollments" stroke="hsl(var(--accent))" dot={false} strokeWidth={2} />
              </LineChart>
            </ChartContainer>
          )}
        </CardContent>
