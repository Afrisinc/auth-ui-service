        ...
        <div className="flex gap-1 flex-wrap">
          {account.products && account.products.length > 0 ? (
            account.products.map((p) => (
              <div key={p.id} className="flex items-center gap-1 rounded-md bg-secondary/50 px-2 py-1 text-xs" title={`${p.product?.name || "Unknown"} (${p.product?.code || p.product_id})`}>
    ...
    },
    {
      ...
      render: (_, account) => (
        <div className="flex justify-end gap-1">
          <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setSelectedAccount(account); setDetailsSheetOpen(true); }}>
