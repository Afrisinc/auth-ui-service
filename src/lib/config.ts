...
export async function loadRuntimeConfig(): Promise<RuntimeConfig> {
  ...
  try {
    ...
    if (config.serverUrl) {
      const source = import.meta.env.VITE_API_URL
        ? "build-time VITE_API_URL"
        : "config.json";
      // console.log(`[Config] Using ${source}:`, config.serverUrl);
    } else {
      console.warn(
        "[Config] serverUrl not configured. Set VITE_API_URL env var at build time."
      );
    }
    ...
    console.warn(
      "[Config] Could not load config.json, falling back to build-time VITE_API_URL",
      error
    );
    config = {
    ...
    if (config.serverUrl) {
      ...
      console.warn(
        "[Config] Warning: VITE_API_URL not configured. API calls will fail."
      );
    }
    return config;
  }
